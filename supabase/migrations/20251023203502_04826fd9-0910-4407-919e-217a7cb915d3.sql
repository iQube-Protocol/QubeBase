-- First, drop ALL existing policies on registry_mirror tables
DO $$ 
DECLARE
    pol RECORD;
BEGIN
    FOR pol IN 
        ...
        FROM pg_policies 
        WHERE schemaname = 'registry_mirror' 
        AND tablename IN ('templates', 'instances', 'proofs')
    LOOP
        EXECUTE format('DROP POLICY IF EXISTS %I ON registry_mirror.%I', pol.policyname, pol.tablename);
    END LOOP;
END $$;
